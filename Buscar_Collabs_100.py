...
def cargar_archivo_excel():
    ...
    # Obtener el nombre del autor y apellidos del nombre del archivo
    autor_info = "_".join(archivo_seleccionado.split('_')[-3:]).split('.')[0]
...
def exportar_resultados(df_unificacion, df_agrupacion, df_no_agrupados, autor_info):
    ...
    # Incorporar el nombre del autor en el archivo exportado
    export_filename = f'100_Agrup_Unif_{autor_info}.xlsx'
    with pd.ExcelWriter(export_filename, engine='openpyxl', mode='w') as writer:
...
def main():
    df_colaboraciones, autor_info = cargar_archivo_excel()

    if df_colaboraciones is None:
        print("Error al cargar el archivo de Excel. Intente nuevamente.")
        return

    buscar_y_sumar_por_identificadores(df_colaboraciones, autor_info)

def buscar_y_sumar_por_identificadores(df_colaboraciones, autor_info):
    grupos_resultados = []
    obras_grupos = []
    obras_no_agrupadas = []
    ...
    for _, row in df_colaboraciones.iterrows():
        # Crear una clave de agrupación basada en todos los identificadores clave
        key = tuple(row[col] for col in ['ISRC'] + columnas_identificadores if pd.notna(row[col]))
        if key:
            tree.insert(key, row)
    ...
    exportar_resultados(pd.DataFrame(grupos_resultados), pd.DataFrame(obras_grupos), pd.DataFrame(obras_no_agrupadas), autor_info)
